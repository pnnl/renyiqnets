"""
Title: Training of a Quantum Boltzmann Machine using Renyi Divergence
...
class bm_model:
        ...
        def ad_recursion(self, p, k):
            prod_terms = [(w*p_op) for w,p_op in 
                          zip(self.weights, self.pauli_ops)]
            H_minus = ((-1)*sum(prod_terms))
            term = ((H_minus*self.pauli_ops[k]) - self.pauli_ops[k]*(H_minus))
            if p == 0:
                return self.pauli_ops[k]
            elif p == 1:
                return term
            else:
                for _ in range(p-1):
                    term = H_minus*term - term*H_minus
                return term

#            if p == 1:
#                return (H_minus*self.pauli_ops[k] -\
#                        self.pauli_ops[k]*(H_minus))
#            else:
#                return ( H_minus*self.ad_recursion(p-1, k) -\
#                        self.ad_recursion(p-1, k)*H_minus )

        def ad_recursion_tf(self, p, k):
            # a bit faster
            prod_terms = [(w*p_op) for w,p_op in 
                          zip(self.weights, self.pauli_ops)]
            H_minus = ((-1)*sum(prod_terms))
            dims = self.pauli_ops[k].dims
            term = multidot_tf([H_minus ,self.pauli_ops[k]]) -\
                multidot_tf([self.pauli_ops[k], H_minus])
            if p == 0:
                return self.pauli_ops[k]
            elif p == 1:
                return Qobj(term.numpy(), dims=dims)
            else:
                for _ in range(p-1):
                    term = multidot_tf([H_minus, term]) -\
                        multidot_tf([term, H_minus])
                return Qobj(term.numpy(), dims=dims)

        def grad(self, k, p_tol=10):
            if self.vis_size != self.size:
                id_hid = tensor([identity(2) for _ in range(self.size-self.vis_size)])
            prod_terms = [(w*p_op) for w, p_op in 
                          zip(self.weights, self.pauli_ops)]
            H_minus = ((-1)*sum(prod_terms))
            H_minus_exp = H_minus.expm()
            sigma_v = self.vis_forward()
            anti_comm = sigma_v*self.rho_inv + self.rho_inv*sigma_v
            # here is the bottleneck
            ads = [(1/factorial(p_idx+1))*self.ad_recursion(p=p_idx, k=k) 
                   for p_idx in range(p_tol)]

            if self.vis_size == self.size:
                terms = [( ad*(H_minus_exp)*anti_comm ).tr() for ad in ads]
            else:
                terms = [( ad*(H_minus_exp)*(tensor([anti_comm, id_hid])) ).tr()
                         for ad in ads]
            norm_term=((sigma_v*sigma_v*self.rho_inv).tr())*(H_minus_exp.tr())
            series = ((-1)/(norm_term))*sum(terms) +\
                      2*(((self.pauli_ops[k]*H_minus_exp).tr())\
                         /(H_minus_exp.tr()))
            return series

#       Speedup attempt
#        def grad_tf(self, k, p_tol=10):
#            if self.vis_size != self.size:
#                id_hid = tensor([identity(2) for _ in range(self.size-self.vis_size)])
#            prod_terms = [(w*p_op) for w, p_op in 
#                          zip(self.weights, self.pauli_ops)]
...
#            ads_fn = lambda p_idx: (1/factorial(p_idx.numpy()+1))*\
#                                     self.ad_recursion_tf(p=p_idx, k=k)
#            ads = tf.map_fn(ads_fn, tf.range(p_tol)).numpy().tolist()
#
#            if self.vis_size == self.size:
#                terms = [( ad*(H_minus_exp)*anti_comm ).tr() for ad in ads]
#            else:
...
#            return series
        ...
        def backward(self, l2_reg_penalty = 0.0):
            grads = [self.grad(k)+l2_reg_penalty*2*w for k,w in 
                     zip(range(len(self.weights)), self.weights)]
            prod_terms = [(w*p_op) for w, p_op in 
                          zip(self.weights, self.pauli_ops)]
            print(' ')
            print('Max grad norm: ', np.abs(np.array(grads).real).max())
            print(' ')
...
##   Trace distance as a measure of test accuracy
        tdist = tracedist(sigma_v, rho)
        fid = fidelity(sigma_v, rho)
        csv_writer.writerow([loss.real, loss, tdist, fid, grad_norm])
    f.close()
